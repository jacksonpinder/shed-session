...
- ~~**Loop lane redesign for consistency and less vertical space**~~ — DONE. Rail-style 4px bars packed into rows (`assignLanes`) with one chip band per row (interleaved, mirroring `PDFViewer` margin bars/chips). Chips centered on their bar, offset apart on collision, truncated as a last resort (`src/lib/placeChips.ts`, unit-tested). Bars + chips both toggle select/deselect; each chip has an always-visible ⋯ menu (Rename → name modal, Delete → immediate + undo toast). Loop creation now opens the name modal (default `Loop N`); cancel discards the unsaved region. Collapse/peek + chevron removed. Double-click a chip also opens rename. → `LoopLaneStrip.tsx`, `PlayerDock.tsx`
